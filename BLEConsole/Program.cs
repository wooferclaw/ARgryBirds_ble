...
namespace BLEConsole
{
    class Program
    {
        ...
        const string _endpoint = "https://en1lc45zkcr5j.x.pipedream.net"; 
        ...
        private static void StartServer()
        {
            try
            {
                using (WebApp.Start<Startup>("http://+:3333"))
                {
                    Console.WriteLine($"Service hosted at http://+:3333 listening for requests from https://arngry.herokuapp.com/");
                    Thread.Sleep(-1);
                }
            }
            catch (Exception e)
        ...
        static void Main(string[] args)
        {
            ...
            ListDevices(String.Empty);
            Thread.Sleep(100);
            _exitCode += OpenDevice("HMSoft").Result;
            _exitCode += SetService("#02").Result;
            _exitCode += SubscribeToCharacteristic("#02/#0").Result;
        ...
        static void Characteristic_ValueChanged(GattCharacteristic sender, GattValueChangedEventArgs args)
        {
            ...
            if (_primed)
            {
                ...
                {
                    { sender.Uuid.ToString(), newValue },
                };

                var content = new FormUrlEncodedContent(value);
                var response = client.PostAsync(_endpoint, content).Result;
                var responseString = response.Content.ReadAsStringAsync();
                Console.Write($"Send POST request {sender.Uuid}: {newValue}\n {responseString} ");
            }
            else  _primed = true;
            Console.Write($"POST request not send, _primed = true");
        }

        static Tuple<string,string> Characteristic_ValueChanged(GattCharacteristic sender, GattValueChangedEventArgs args, bool asTuple = false)
        {
            if (_primed)
            {
                var newValue = Utilities.FormatValue(args.CharacteristicValue, _dataFormat);
                ...
                if (asTuple)
                    return new Tuple<string, string>( sender.Uuid.ToString(), newValue) ;
            }
            else _primed = true;

            return new Tuple<string, string>(sender.Uuid.ToString(), null); ;
        }
